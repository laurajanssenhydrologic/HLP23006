...
from data_structures.to_dhydro_helpers import fm_to_dhydro
...
class FMData:
    ...
    def merge_ext_files(self, extforcefile: ExtModel) -> None:
        assert isinstance(self.fmmodel, FMModel)

        if (not hasattr(self.fmmodel.external_forcing, "extforcefilenew")) or (
            self.fmmodel.external_forcing.extforcefilenew is None
        ):
            self.fmmodel.external_forcing.extforcefilenew = extforcefile
        else:
            if hasattr(extforcefile, "lateral"):
                if (not hasattr(self.fmmodel.external_forcing.extforcefilenew, "lateral")) or (
                    self.fmmodel.external_forcing.extforcefilenew.lateral is None
                ):
                    self.fmmodel.external_forcing.extforcefilenew.lateral = extforcefile.lateral
                else:
                    for lateral in extforcefile.lateral:
                        self.fmmodel.external_forcing.extforcefilenew.lateral.append(lateral)
            if hasattr(extforcefile, "boundary"):
                if (not hasattr(self.fmmodel.external_forcing.extforcefilenew, "boundary")) or (
                    self.fmmodel.external_forcing.extforcefilenew.boundary is None
                ):
                    self.fmmodel.external_forcing.extforcefilenew.boundary = extforcefile.boundary
                else:
                    for boundary in extforcefile.boundary:
                        self.fmmodel.external_forcing.extforcefilenew.boundary.append(boundary)
    ...
    def to_dhydro(self, config: str, output_folder: str, defaults: str = "defaults", write=True):
        ...
        self.fmmodel = fm_to_dhydro(
            config=config, dm=self.dm, features=self.features, output_folder=output_folder
        )

        # if write:
        #     try:
        #         self.super().write_dimr(output_folder=output_folder)
    ...
    def write_dimr(self, output_path: Path):
        """ """
        if (not hasattr(self, "fmmodel")) or (self.fmmodel is None):
            print("No FM model present")
            return None
        else:
            fm = self.fmmodel
        ...
        dimr.component.append(
            FMComponent(
                name="DFM", workingDir=output_path / FM_FOLDER, model=fm, inputfile=fm.filepath
            )
        )
