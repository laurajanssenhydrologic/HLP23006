...
class RasterPart:
    ...
    def get_xy_range(self):
        x = np.linspace(
            self.lowerleft[0], self.upperright[0], (self.xmax - self.xmin), endpoint=False
        )
        y = np.linspace(
            self.lowerleft[1], self.upperright[1], (self.ymax - self.ymin), endpoint=False
        )[::-1]
...
def raster_in_parts(f: rasterio.io.DatasetReader, ncols: int, nrows: int, facedata) -> RasterPart:
    ...
    for ix, iy in product(range(nx), range(ny)):
        ...
        part = RasterPart(
            f, xmin=xparts[ix], ymin=yparts[iy], xmax=xparts[ix + 1], ymax=yparts[iy + 1]
        )
...
def raster_stats_fine_cells(
    rasterpath: Union[str, Path], facedata, stats=["mean"], window_size=250
):
    """
    ...
    with rasterio.open(rasterpath, "r") as f:
        ...
        parts = raster_in_parts(f, ncols=window_size, nrows=window_size, facedata=facedata)
        ...
        for prt in parts:
            ...
            with rasterio.open(outpath, "w" if first else "r+", **out_meta) as dst:
                ...
                if not first:
                    ...
                    first = False
                wdep_subgr[valid] = np.maximum(wlev_subgr - arr, 0).astype(out_meta["dtype"])[
                    valid
                ]
